...
def find_matching_keywords(text, keywords_dict):
    ...
    for category, keywords in keywords_dict.items():
        for keyword in keywords:
            pattern = rf'\b{re.escape(keyword)}s?\b' # This regex is used to search for keywords in the text. The re.escape(keyword) ensures that any special characters in the keyword are escaped properly, and the s? part accounts for plural forms (optional 's'). The regex also uses word boundaries (\b) to match complete words only, avoiding partial matches.
...
def process_srt_files(srt_directory, output_directory):
    for filename in os.listdir(srt_directory):
        if filename.endswith('.srt'):
            ...
            try:
                ...
                if not subs:  # Check if parsing returned an empty list
                    print(f"⚠️ Skipping {filename}: No subtitles found.")
                    continue

                df = pd.DataFrame(subs)

                if 'text' not in df.columns or df.empty:  # Ensure 'text' column exists
    ...
    print("🎬 SRT processing complete.")


# Main script
srt_directory = r"C:\Users\elzbe\DATA\SRT"
output_directory = r"C:\Users\elzbe\DATA\CSV"
filtered_output_directory = r"C:\Users\elzbe\DATA\Filtered_CSV"
keywords_directory = r"C:\Users\elzbe\DATA\JSON"
